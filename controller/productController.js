...
async function addProduct(req,res) {
    ...
    try {
        ...
        let respsData = await productModel.create({ product_name, price, brand, category, stock,thumbnail:url+"/uploads/products/"+req.files[0].originalname, description, discountPercentage, adder: req.data.id })
        if (!respsData) {
